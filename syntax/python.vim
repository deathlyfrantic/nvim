...
if s:Python2Syntax()
  ...
  syn region pythonDocString  start=+^\s*[uU]"""+ end=+"""+ keepend contains=pythonBytesEscape,pythonBytesEscapeError,pythonUniEscape,pythonUniEscapeError,pythonDocTest2,pythonSpaceError,@Spell
else
  " Python 3 strings
  ...
  syn region pythonDocString start=+^\s*"""+ end=+"""+ keepend contains=pythonBytesEscape,pythonBytesEscapeError,pythonUniEscape,pythonUniEscapeError,pythonDocTest2,pythonSpaceError,@Spell
endif
...
if s:Python2Syntax()
  ...
  syn region pythonDocString    start=+^\s*[uU][rR]"""+ end=+"""+ keepend contains=pythonUniRawEscape,pythonUniRawEscapeError,pythonDocTest2,pythonSpaceError,@Spell
...
if s:Python2Syntax()
  ...
  syn region pythonDocString  start=+^\s*[rR]"""+ end=+"""+ keepend contains=pythonDocTest2,pythonSpaceError,@Spell
...
if s:Enabled("g:python_highlight_string_formatting")
  ...
  if s:Python2Syntax()
    syn match pythonStrFormatting	"%\%(([^)]\+)\)\=[-#0 +]*\d*\%(\.\d\+\)\=[hlL]\=[diouxXeEfFgGcrs%]" contained containedin=pythonString,pythonUniString,pythonDocString,pythonUniRawString,pythonRawString
    syn match pythonStrFormatting	"%[-#0 +]*\%(\*\|\d\+\)\=\%(\.\%(\*\|\d\+\)\)\=[hlL]\=[diouxXeEfFgGcrs%]" contained containedin=pythonString,pythonUniString,pythonDocString,pythonUniRawString,pythonRawString
  else
    syn match pythonStrFormatting	"%\%(([^)]\+)\)\=[-#0 +]*\d*\%(\.\d\+\)\=[hlL]\=[diouxXeEfFgGcrs%]" contained containedin=pythonString,pythonDocString,pythonRawString
    syn match pythonStrFormatting	"%[-#0 +]*\%(\*\|\d\+\)\=\%(\.\%(\*\|\d\+\)\)\=[hlL]\=[diouxXeEfFgGcrs%]" contained containedin=pythonString,pythonDocString,pythonRawString
  endif
endif
...
if s:Enabled("g:python_highlight_string_format")
  " str.format syntax
  if s:Python2Syntax()
    syn match pythonStrFormat "{{\|}}" contained containedin=pythonString,pythonUniString,pythonDocString,pythonUniRawString,pythonRawString
    syn match pythonStrFormat	"{\%(\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*\|\d\+\)\=\%(\.\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*\|\[\%(\d\+\|[^!:\}]\+\)\]\)*\%(![rsa]\)\=\%(:\%({\%(\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*\|\d\+\)}\|\%([^}]\=[<>=^]\)\=[ +-]\=#\=0\=\d*,\=\%(\.\d\+\)\=[bcdeEfFgGnosxX%]\=\)\=\)\=}" contained containedin=pythonString,pythonUniString,pythonDocString,pythonUniRawString,pythonRawString
  else
    syn match pythonStrFormat "{{\|}}" contained containedin=pythonString,pythonDocString,pythonRawString
    syn match pythonStrFormat	"{\%(\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*\|\d\+\)\=\%(\.\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*\|\[\%(\d\+\|[^!:\}]\+\)\]\)*\%(![rsa]\)\=\%(:\%({\%(\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*\|\d\+\)}\|\%([^}]\=[<>=^]\)\=[ +-]\=#\=0\=\d*,\=\%(\.\d\+\)\=[bcdeEfFgGnosxX%]\=\)\=\)\=}" contained containedin=pythonString,pythonDocString,pythonRawString
  endif
endif
...
if s:Enabled("g:python_highlight_string_templates")
  ...
  if s:Python2Syntax()
    syn match pythonStrTemplate	"\$\$" contained containedin=pythonString,pythonUniString,pythonDocString,pythonUniRawString,pythonRawString
    syn match pythonStrTemplate	"\${[a-zA-Z_][a-zA-Z0-9_]*}" contained containedin=pythonString,pythonUniString,pythonDocString,pythonUniRawString,pythonRawString
    syn match pythonStrTemplate	"\$[a-zA-Z_][a-zA-Z0-9_]*" contained containedin=pythonString,pythonUniString,pythonDocString,pythonUniRawString,pythonRawString
  else
    syn match pythonStrTemplate	"\$\$" contained containedin=pythonString,pythonDocString,pythonRawString
    syn match pythonStrTemplate	"\${[a-zA-Z_][a-zA-Z0-9_]*}" contained containedin=pythonString,pythonDocString,pythonRawString
    syn match pythonStrTemplate	"\$[a-zA-Z_][a-zA-Z0-9_]*" contained containedin=pythonString,pythonDocString,pythonRawString
  endif
endif
...
if s:Enabled("g:python_highlight_builtin_objs")
  ...
  syn keyword pythonBuiltinObj	None Ellipsis NotImplemented
...
if s:Enabled("g:python_highlight_builtin_funcs")
  if s:Python2Syntax()
    syn match pythonBuiltinFunc "\.\@<!\<apply\>"
    syn match pythonBuiltinFunc "\.\@<!\<basestring\>"
    syn match pythonBuiltinFunc "\.\@<!\<buffer\>"
    syn match pythonBuiltinFunc "\.\@<!\<callable\>"
    syn match pythonBuiltinFunc "\.\@<!\<coerce\>"
    syn match pythonBuiltinFunc "\.\@<!\<execfile\>"
    syn match pythonBuiltinFunc "\.\@<!\<file\>"
    syn match pythonBuiltinFunc "\.\@<!\<help\>"
    syn match pythonBuiltinFunc "\.\@<!\<intern\>"
    syn match pythonBuiltinFunc "\.\@<!\<long\>"
    syn match pythonBuiltinFunc "\.\@<!\<raw_input\>"
    syn match pythonBuiltinFunc "\.\@<!\<reduce\>"
    syn match pythonBuiltinFunc "\.\@<!\<reload\>"
    syn match pythonBuiltinFunc "\.\@<!\<unichr\>"
    syn match pythonBuiltinFunc "\.\@<!\<unicode\>"
    syn match pythonBuiltinFunc "\.\@<!\<xrange\>"
    if s:Enabled("g:python_print_as_function")
      syn keyword pythonBuiltinFunc	print
    endif
  else
    syn match pythonBuiltinFunc "\.\@<!\<ascii\>"
    syn match pythonBuiltinFunc "\.\@<!\<exec\>"
    syn match pythonBuiltinFunc "\.\@<!\<memoryview\>"
    syn keyword pythonBuiltinFunc	print
  endif
  syn match pythonBuiltinFunc "\.\@<!\<__import__\>"
  syn match pythonBuiltinFunc "\.\@<!\<abs\>"
  syn match pythonBuiltinFunc "\.\@<!\<all\>"
  syn match pythonBuiltinFunc "\.\@<!\<any\>"
  syn match pythonBuiltinFunc "\.\@<!\<bin\>"
  syn match pythonBuiltinFunc "\.\@<!\<bool\>"
  syn match pythonBuiltinFunc "\.\@<!\<bytearray\>"
  syn match pythonBuiltinFunc "\.\@<!\<bytes\>"
  syn match pythonBuiltinFunc "\.\@<!\<chr\>"
  syn match pythonBuiltinFunc "\.\@<!\<classmethod\>"
  syn match pythonBuiltinFunc "\.\@<!\<cmp\>"
  syn match pythonBuiltinFunc "\.\@<!\<compile\>"
  syn match pythonBuiltinFunc "\.\@<!\<complex\>"
  syn match pythonBuiltinFunc "\.\@<!\<delattr\>"
  syn match pythonBuiltinFunc "\.\@<!\<dict\>"
  syn match pythonBuiltinFunc "\.\@<!\<dir\>"
  syn match pythonBuiltinFunc "\.\@<!\<divmod\>"
  syn match pythonBuiltinFunc "\.\@<!\<enumerate\>"
  syn match pythonBuiltinFunc "\.\@<!\<eval\>"
  syn match pythonBuiltinFunc "\.\@<!\<filter\>"
  syn match pythonBuiltinFunc "\.\@<!\<float\>"
  syn match pythonBuiltinFunc "\.\@<!\<format\>"
  syn match pythonBuiltinFunc "\.\@<!\<frozenset\>"
  syn match pythonBuiltinFunc "\.\@<!\<getattr\>"
  syn match pythonBuiltinFunc "\.\@<!\<globals\>"
  syn match pythonBuiltinFunc "\.\@<!\<hasattr\>"
  syn match pythonBuiltinFunc "\.\@<!\<hash\>"
  syn match pythonBuiltinFunc "\.\@<!\<hex\>"
  syn match pythonBuiltinFunc "\.\@<!\<id\>"
  syn match pythonBuiltinFunc "\.\@<!\<input\>"
  syn match pythonBuiltinFunc "\.\@<!\<int\>"
  syn match pythonBuiltinFunc "\.\@<!\<isinstance\>"
  syn match pythonBuiltinFunc "\.\@<!\<issubclass\>"
  syn match pythonBuiltinFunc "\.\@<!\<iter\>"
  syn match pythonBuiltinFunc "\.\@<!\<len\>"
  syn match pythonBuiltinFunc "\.\@<!\<list\>"
  syn match pythonBuiltinFunc "\.\@<!\<locals\>"
  syn match pythonBuiltinFunc "\.\@<!\<map\>"
  syn match pythonBuiltinFunc "\.\@<!\<max\>"
  syn match pythonBuiltinFunc "\.\@<!\<min\>"
  syn match pythonBuiltinFunc "\.\@<!\<next\>"
  syn match pythonBuiltinFunc "\.\@<!\<object\>"
  syn match pythonBuiltinFunc "\.\@<!\<oct\>"
  syn match pythonBuiltinFunc "\.\@<!\<open\>"
  syn match pythonBuiltinFunc "\.\@<!\<ord\>"
  syn match pythonBuiltinFunc "\.\@<!\<pow\>"
  syn match pythonBuiltinFunc "\.\@<!\<property\>"
  syn match pythonBuiltinFunc "\.\@<!\<range\>"
  syn match pythonBuiltinFunc "\.\@<!\<repr\>"
  syn match pythonBuiltinFunc "\.\@<!\<reversed\>"
  syn match pythonBuiltinFunc "\.\@<!\<round\>"
  syn match pythonBuiltinFunc "\.\@<!\<set\>"
  syn match pythonBuiltinFunc "\.\@<!\<setattr\>"
  syn match pythonBuiltinFunc "\.\@<!\<slice\>"
  syn match pythonBuiltinFunc "\.\@<!\<sorted\>"
  syn match pythonBuiltinFunc "\.\@<!\<staticmethod\>"
  syn match pythonBuiltinFunc "\.\@<!\<str\>"
  syn match pythonBuiltinFunc "\.\@<!\<sum\>"
  syn match pythonBuiltinFunc "\.\@<!\<super\>"
  syn match pythonBuiltinFunc "\.\@<!\<tuple\>"
  syn match pythonBuiltinFunc "\.\@<!\<type\>"
  syn match pythonBuiltinFunc "\.\@<!\<vars\>"
  syn match pythonBuiltinFunc "\.\@<!\<zip\>"
endif
